	...
	E.Options.args.cooldown.args[db] = {
		...
		args = {
			...
			reverse = {
				...
				get = function(info)
					return (profile(db))[info[#info]]
				end,
				set = function(info, value)
					(profile(db))[info[#info]] = value
					E:UpdateCooldownSettings(db)
				end,
			},
			secondsGroup = {
				...
				args = {
					...
					threshold = {
						...
						step = 1,
						disabled = function()
							return not (profile(db)).override
						end,
						get = function(info)
							return (profile(db))[info[#info]]
						end,
					...
					mmssThreshold = {
						...
						step = 1,
						disabled = function()
							return not (profile(db)).checkSeconds
						end,
					},
					hhmmThreshold = {
						order = 4,
			...
			fontGroup = {
				...
				guiInline = true,
				get = function(info)
					return (profile(db)).fonts[info[#info]]
				end,
				set = function(info, value)
					(profile(db)).fonts[info[#info]] = value
					E:UpdateCooldownSettings(db)
				end,
				disabled = function()
					return not (profile(db)).fonts.enable
				end,
				args = {
					enable = {
						order = 1,
...
E.Options.args.cooldown = {
	...
	childGroups = "tab",
	get = function(info)
		return E.db.cooldown[info[#info]]
	end,
	set = function(info, value)
