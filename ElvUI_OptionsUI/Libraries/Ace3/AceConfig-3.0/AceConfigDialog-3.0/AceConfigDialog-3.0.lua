...
local function OptionOnMouseOver(widget, event)
	...
	-- modified by ElvUI
	if opt.descStyle and opt.descStyle ~= "tooltip" then
		return
	end
	...
	if descText or usageText or userText or softText or bigText then
		...
		GameTooltip:SetText(name, 1, 0.82, 0, true)
			...
			if v.type == "group" then
				...
				elseif v.type == "toggle" then
					...
					if control.textWidth and control.frame and control.text then
						...
						control.customWidth = (textWidth >= width_multiplier and textWidth <= width_multiplier * 1.5)
							and textWidth
					end
					...
					if control then
						...
						for i = 1, #valuesort do
							...
							if dragdrop then
								...
								local customWidth = control.customWidth
									or GetOptionsMemberValue("customWidth", v, options, path, appName)
								if customWidth then
...
local function TreeOnButtonEnter(widget, event, uniquevalue, button)
	...
	if type(desc) == "string" then
		...
		GameTooltip:SetText(name, 1, 0.82, 0, 1)
