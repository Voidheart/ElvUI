...
E.Options.args.databars = {
	...
	childGroups = "tab",
	get = function(info)
		return E.db.databars[info[#info]]
	end,
	set = function(info, value)
		E.db.databars[info[#info]] = value
	end,
	args = {
		intro = {
			order = 1,
		...
		experience = {
			...
			name = L["XPBAR_LABEL"],
			get = function(info)
				return mod.db.experience[info[#info]]
			end,
			set = function(info, value)
				mod.db.experience[info[#info]] = value
			...
			args = {
				...
				width = {
					...
					name = L["Width"],
					min = 5,
					max = ceil(GetScreenWidth() or 800),
					step = 1,
				},
				height = {
					order = 10,
					...
					name = L["Height"],
					min = 5,
					max = ceil(GetScreenHeight() or 800),
					step = 1,
				},
				font = {
					order = 11,
				...
				questXP = {
					...
					guiInline = true,
					get = function(info)
						return mod.db.experience.questXP[info[#info]]
					end,
					disabled = function()
						return not mod.db.experience.enable or not mod.db.experience.questXP.enable
					end,
					args = {
						enable = {
							order = 1,
							...
							disabled = function()
								return not mod.db.experience.enable
							end,
						},
						color = {
							order = 2,
						...
						tooltip = {
							...
							set = function(info, value)
								mod.db.experience.questXP.tooltip = value
							end,
						},
					},
				},
		...
		petExperience = {
			...
			get = function(info)
				return mod.db.petExperience[info[#info]]
			end,
			set = function(info, value)
				mod.db.petExperience[info[#info]] = value
				mod:PetExperienceBar_UpdateDimensions()
			end,
			args = {
				header = {
					order = 1,
		...
		reputation = {
			...
			name = L["REPUTATION"],
			get = function(info)
				return mod.db.reputation[info[#info]]
			end,
			set = function(info, value)
				mod.db.reputation[info[#info]] = value
				mod:ReputationBar_UpdateDimensions()
			end,
			args = {
				header = {
					order = 1,
