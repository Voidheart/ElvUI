...
function D:Distribute(target, otherServer, isGlobal)
	...
	if not data or not profileKey then
		return
	end
...
function D:CHAT_MSG_ADDON(_, prefix, message, _, sender)
	if not ACECOMMPREFIXES[prefix] or not Downloads[sender] then
		return
	end
	...
	elseif prefix == REPLY_PREFIX then
		...
		if response == "YES" then
			...
			self:SendCommMessage(
				TRANSFER_PREFIX,
				Uploads[profileKey].serialData,
				dist,
				Uploads[profileKey].target,
				"BULK",
				...
				if not ElvDB.profiles[profileKey] then
					...
					E.PopupDialogs.DISTRIBUTOR_CONFIRM = {
						...
						OnShow = function(popup)
							popup.editBox:SetText(profileKey)
...
local function GetProfileData(profileType)
	...
	elseif profileType == "filters" then
		...
		profileData.unitframe.aurafilters =
			E:CopyTable(profileData.unitframe.aurafilters, ElvDB.global.unitframe.aurafilters)
...
E.PopupDialogs.IMPORT_PROFILE_EXISTS = {
	...
	OnShow = function(self)
		self.editBox:SetText(D.profileKey)
