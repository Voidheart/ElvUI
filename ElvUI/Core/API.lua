...
function E:IsDispellableByMe(debuffType)
	if not self.DispelClasses[self.myclass] then
		return
	end
...
do
	...
	local function CompareCPUDiff(showall, minCalls)
				...
				if showall and (calls > minCalls) then
					E:Print(
						"Name("
							...
							.. calls
							.. ") Diff("
							.. (diff > 0 and format("%.3f", diff) or 0)
							.. ")"
					)
				end
		...
		if greatestName then
			E:Print(
				greatestName
					.. " had the CPU usage of: "
					.. (greatestUsage > 0 and format("%.3f", greatestUsage) or 0)
					.. "ms. And has been called "
					.. greatestCalls
					.. " times."
			)
		else
...
function E:StopSpinnerFrame(parent)
	if not parent.SpinnerFrame then
		return
	end
