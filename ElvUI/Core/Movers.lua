...
local function UpdateMover(parent, name, text, overlay, snapOffset, postdrag, shouldDisable, configString)
	if not parent then
		return
	end --If for some reason the parent isnt loaded yet
	if E.CreatedMovers[name].Created then
		return
	end
	...
	if not point2 then -- fallback to the parents point if the setting doesn't exist
		point2, relativeTo2, relativePoint2, xOffset2, yOffset2 =
			point1, relativeTo1, relativePoint1, xOffset1, yOffset1
	end
...
function E:CreateMover(parent, name, text, overlay, snapoffset, postdrag, moverTypes, shouldDisable, configString)
	if not moverTypes then
		moverTypes = "ALL,GENERAL"
	end
...
function E:DisableMover(name)
	if self.DisabledMovers[name] then
		return
	end
...
function E:EnableMover(name)
	if self.CreatedMovers[name] then
		return
	end
