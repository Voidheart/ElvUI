...
function E:FarmMode(msg)
	...
	if
		msg
		and type(tonumber(msg)) == "number"
		and tonumber(msg) <= 500
		and tonumber(msg) >= 20
...
local MassKickMessage =
	"Guild Cleanup Results: Removed all guild members below rank %s, that have a minimal level of %s, and have not been online for at least: %s days."
function E:MassGuildKick(msg)
	...
	for i = 1, GetNumGuildMembers() do
		...
		if not connected then
			...
			if
				days ~= nil
				and ((years > 0 or months > 0 or days >= minDays) and rankIndex >= minRankIndex)
				and note ~= nil
				and officerNote ~= nil
				and (level <= minLevelx)
			then
