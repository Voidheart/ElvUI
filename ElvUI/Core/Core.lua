...
local format, find, match, strrep, strlen, sub, gsub, strjoin =
	string.format, string.find, string.match, strrep, strlen, string.sub, string.gsub, strjoin
--WoW API / Variables
...
function E:Print(...)
	(_G[self.db.general.messageRedirect] or DEFAULT_CHAT_FRAME):AddMessage(
		strjoin(" ", self:ColorizedName("ElvUI", true), ...)
	) -- I put DEFAULT_CHAT_FRAME as a fail safe.
end
...
function E:CheckClassColor(r, g, b)
	...
	for class in pairs(RAID_CLASS_COLORS) do
		if class ~= E.myclass then
			local colorTable = class == "PRIEST" and E.PriestColors
				or (CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[class] or RAID_CLASS_COLORS[class])
...
function E:UpdateMedia()
	if not self.db.general or not self.private.general then
		return
	end --Prevent rare nil value errors
	...
	if self:CheckClassColor(value.r, value.g, value.b) then
		value = E.myclass == "PRIEST" and E.PriestColors
			or (CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[E.myclass] or RAID_CLASS_COLORS[E.myclass])
...
do
	...
	local function SendRecieve(_, event, prefix, message, _, sender)
		if event == "CHAT_MSG_ADDON" then
			...
			if not sender or sender == E.myname then
				return
			end
...
function E:DBConversions()
	do -- <= 6.07
		...
		local healPredictionUnits =
			{ "player", "target", "focus", "pet", "arena", "party", "raid", "raid40", "raidpet" }
...
function E:Initialize()
	...
	if self.db.general.loginmessage then
		...
		if Chat.Initialized then
			msg = select(2, Chat:FindURL("CHAT_MSG_DUMMY", msg))
		end
		print(msg)
	end
