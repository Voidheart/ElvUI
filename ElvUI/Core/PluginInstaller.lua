...
function PI:CreateFrame()
	...
	f.pending.tex:Point("TOPLEFT", 2, -2)
	f.pending.tex:Point("BOTTOMRIGHT", -2, 2)
	...
	f.pending:SetScript("OnEnter", function(self)
		...
		for i = 1, #PI.Installs do
			GameTooltip:AddDoubleLine(
				format("%d. %s", i, (PI.Installs[i].Name or UNKNOWN)),
				i == 1 and format("|cff00FF00%s|r", L["In Progress"]) or format("|cffFF0000%s|r", L["Pending"])
			)
		end
		GameTooltip:Show()
	end)
...
function PI:RunInstall()
	...
	if
		self.Installs[1]
		and not PluginInstallFrame:IsShown()
		and not (ElvUIInstallFrame and ElvUIInstallFrame:IsShown())
	then
		f.StepTitles = nil
