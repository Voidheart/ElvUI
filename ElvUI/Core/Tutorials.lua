...
function E:SpawnTutorialFrame()
	...
	f.disableButton:SetScript("OnShow", function(self)
		self:SetChecked(E.db.hideTutorial)
	end)

	f.disableButton:SetScript("OnClick", function(self)
		E.db.hideTutorial = self:GetChecked()
	end)
	...
	f.hideButton:SetScript("OnClick", function(self)
		E:StaticPopupSpecial_Hide(self:GetParent())
	end)
...
function E:Tutorials(forceShow)
	if (not forceShow and self.db.hideTutorial) or (not forceShow and not self.private.install_complete) then
		return
	end
