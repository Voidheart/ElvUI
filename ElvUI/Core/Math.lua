...
local byte, format, gmatch, gsub, strupper, strsub =
	string.byte, string.format, string.gmatch, string.gsub, strupper, strsub
...
function E:BuildPrefixValues()
	if next(E.ShortPrefixValues) then
...
function E:Delay(delay, func, ...)
	if type(delay) ~= "number" then
		error(
			format("Bad argument #1 to 'Delay' (number expected, got %s)", delay ~= nil and type(delay) or "no value"),
			2
		)
	elseif type(func) ~= "function" then
		error(
			format("Bad argument #2 to 'Delay' (function expected, got %s)", func ~= nil and type(func) or "no value"),
			2
		)
	end
	...
	elseif s < HOUR then
		if mmss and s < mmss then
			...
			if hhmm and s < (hhmm * MINUTE) and 60 < minutes then
				return s / HOUR,
					6,
					minutes > 1 and (s - (minutes * MINUTE - HALFMINUTEISH)) or (s - MINUTEISH),
					minutes % MINUTE
			else
