...
do
	...
	function E:HelloKittyFixCheck(secondCheck)
		local t = self.db.tempSettings
		if not t and not secondCheck then
			t = self.db.general
		end
		if t and t.backdropcolor then
			return self:Round(t.backdropcolor.r, 2) == 0.87
				and self:Round(t.backdropcolor.g, 2) == 0.3
				and self:Round(t.backdropcolor.b, 2) == 0.74
		end
	end
