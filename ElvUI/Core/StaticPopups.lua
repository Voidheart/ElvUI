...
E.PopupDialogs.DISABLE_INCOMPATIBLE_ADDON = {
	...
	OnCancel = function()
		...
		E:StaticPopup_Show(
			"INCOMPATIBLE_ADDON",
			E.PopupDialogs.INCOMPATIBLE_ADDON.addon,
			E.PopupDialogs.INCOMPATIBLE_ADDON.module
		)
	end,
	button1 = L["I Swear"],
	button2 = DECLINE,
...
function E:Contruct_StaticPopups()
	...
	for index = 1, MAX_STATIC_POPUPS do
		E.StaticPopupFrames[index] =
			CreateFrame("Frame", "ElvUI_StaticPopup" .. index, E.UIParent, "StaticPopupTemplate")
		...
		for k = 1, _G[name .. "EditBox"]:GetNumRegions() do
			...
			if region and region:IsObjectType("Texture") then
				if
					region:GetTexture() == "Interface\\ChatFrame\\UI-ChatInputBorder-Left"
					or region:GetTexture() == "Interface\\ChatFrame\\UI-ChatInputBorder-Right"
				then
					region:Kill()
				end
			end
