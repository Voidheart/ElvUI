...
SlashCmdList.FRAME = function(frame)
	...
	printNoTimestamp(
		format("Name: |cffFFD100%s|r; ObjectType: |cffFFD100%s|r", frame:GetName() or "nil", frame:GetObjectType())
	)
	printNoTimestamp(format("Parent: |cffFFD100%s|r", parentName or (parent and tostring(parent)) or "nil"))

	if frame.GetFrameStrata then
		printNoTimestamp(
			format("Strata: |cffFFD100%s|r; FrameLevel: |cffFFD100%d|r", frame:GetFrameStrata(), frame:GetFrameLevel())
		)
	else
	...
	if frame.GetScale then
		printNoTimestamp(
			format(
				"Width: |cffFFD100%.0f|r; Height: |cffFFD100%.0f|r; Scale: |cffFFD100%s|r",
				frame:GetWidth(),
				frame:GetHeight(),
				frame:GetScale()
			)
		...
		printNoTimestamp(
			format("Width: |cffFFD100%.0f|r; Height: |cffFFD100%.0f|r", frame:GetWidth(), frame:GetHeight())
		)
	end
	...
	for i = 1, frame:GetNumPoints() do
		...
		if point == relativePoint and relativeTo == parent then
			...
			printNoTimestamp(
				format('Point %d: |cffFFD100"%s", %s, "%s", %.0f, %.0f|r', i, point, relativeName, relativePoint, x, y)
			)
		end
	end
