...
function AB:PositionAndSizeBar(barName)
	...
	local barWidth = (size * (buttonsPerRow * widthMult))
		+ ((buttonSpacing * (buttonsPerRow - 1)) * widthMult)
		+ (buttonSpacing * (widthMult - 1))
		+ (sideSpacing * 2)
	local barHeight = (size * (numColumns * heightMult))
		+ ((buttonSpacing * (numColumns - 1)) * heightMult)
		+ (buttonSpacing * (heightMult - 1))
		+ (sideSpacing * 2)
	bar:Width(barWidth)
	...
	for i = 1, NUM_ACTIONBAR_BUTTONS do
		...
		self:StyleButton(
			button,
			nil,
			(self.LBFGroup or self.MSQGroup) and E.private.actionbar.lbf.enable and true or nil
		)
	end
...
function AB:UpdateVehicleLeave()
	...
	self.vehicle:Point(
		...
		Minimap,
		pos,
		E.db.general.minimap.icons.vehicleLeave.xOffset,
		E.db.general.minimap.icons.vehicleLeave.yOffset
	)
...
function AB:UpdateBar1Paging()
	...
	if (E.private.actionbar.enable ~= true or InCombatLockdown()) or not self.isInitialized then
		return
	end
	...
	if
		(self.db.bar2.enabled and not bar2Option:GetChecked()) or (not self.db.bar2.enabled and bar2Option:GetChecked())
	then
		bar2Option:Click()
	end

	if
		(self.db.bar3.enabled and not bar3Option:GetChecked()) or (not self.db.bar3.enabled and bar3Option:GetChecked())
	then
		bar3Option:Click()
	end
	...
	elseif
		(self.db.bar4.enabled and not bar4Option:GetChecked()) or (not self.db.bar4.enabled and bar4Option:GetChecked())
	then
		bar4Option:Click()
	elseif
		(self.db.bar5.enabled and not bar5Option:GetChecked()) or (not self.db.bar5.enabled and bar5Option:GetChecked())
	then
		bar5Option:Click()
	end
end
...
local function OnCooldownUpdate(_, button, start, duration)
	...
	if duration and duration > 1.5 then
		...
		if
			(E.db.cooldown.enable and AB.db.cooldown.reverse)
			or (not E.db.cooldown.enable and not AB.db.cooldown.reverse)
		then
