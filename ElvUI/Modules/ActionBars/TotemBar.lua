...
function AB:MultiCastFlyoutFrame_ToggleFlyout(frame, type, parent)
	...
	frame:Height(
		((AB.db.barTotem.buttonsize + AB.db.barTotem.flyoutSpacing) * numButtons)
...
function AB:UpdateTotemBindings()
	...
	MultiCastSummonSpellButtonHotKey:FontTemplate(
		LSM:Fetch("font", self.db.font),
		self.db.fontSize,
		self.db.fontOutline
	)
	...
	MultiCastRecallSpellButtonHotKey:FontTemplate(
		LSM:Fetch("font", self.db.font),
		self.db.fontSize,
		self.db.fontOutline
	)
...
function AB:CreateTotemBar()
	...
	hooksecurefunc(
		...
		"SetPoint",
		function(self, point, attachTo, anchorPoint, xOffset, yOffset)
