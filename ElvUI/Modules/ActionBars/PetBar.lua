...
function AB:UpdatePet(event, unit)
	if (event == "UNIT_FLAGS" or event == "UNIT_AURA") and unit ~= "pet" then
		return
	end
	...
	for i = 1, NUM_PET_ACTION_SLOTS, 1 do
		...
		if
			isActive --[[and name ~= "PET_ACTION_FOLLOW"]]
		then
		...
		if
			not PetHasActionBar() and texture --[[and name ~= "PET_ACTION_FOLLOW"]]
		then
			PetActionButton_StopFlash(button)
			SetDesaturation(icon, 1)
			button:SetChecked(0)
...
function AB:PositionAndSizeBarPet()
	...
	local barWidth = (size * (buttonsPerRow * widthMult))
		...
		+ ((self.db.barPet.backdrop == true and (E.Border + backdropSpacing) or E.Spacing) * 2)
