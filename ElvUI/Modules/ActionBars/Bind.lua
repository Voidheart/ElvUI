...
function AB:BindUpdate(button, spellmacro)
	if not bind.active or InCombatLockdown() then
		return
	end
	...
	if spellmacro == "MACRO" then
		...
		if #bind.button.bindings == 0 then
			GameTooltip:AddLine(L["No bindings set."], 0.6, 0.6, 0.6)
		else
			GameTooltip:AddDoubleLine(L["Binding"], L["Key"], 0.6, 0.6, 0.6, 0.6, 0.6, 0.6)
			for i = 1, #bind.button.bindings do
	...
	elseif spellmacro == "SHAPESHIFT" or spellmacro == "PET" then
		...
		if not bind.button.id or bind.button.id < 1 or bind.button.id > (spellmacro == "SHAPESHIFT" and 10 or 12) then
			...
			bind.button.bindstring = (spellmacro == "SHAPESHIFT" and "SHAPESHIFTBUTTON" or "BONUSACTIONBUTTON")
				.. bind.button.id
		end
		...
		GameTooltip:SetScript("OnHide", function(tt)
			...
			if #bind.button.bindings == 0 then
				tt:AddLine(L["No bindings set."], 0.6, 0.6, 0.6)
			else
				tt:AddDoubleLine(L["Binding"], L["Key"], 0.6, 0.6, 0.6, 0.6, 0.6, 0.6)
				for i = 1, #bind.button.bindings do
		...
		if
			(not bind.button.action or bind.button.action < 1 or bind.button.action > 132)
			and not bind.button.keyBoundTarget
		then
			bind.button.bindstring = "CLICK " .. bind.button.name .. ":LeftButton"
...
function AB:RegisterButton(button, override)
	...
	if
		button.IsProtected
		and button.GetObjectType
		and button.GetScript
		and button:GetObjectType() == "CheckButton"
	...
	then
		...
		if script == secureOnClick or override then
			...
			if script == shapeshift then
				button:HookScript("OnEnter", function(b)
					self:BindUpdate(b, "SHAPESHIFT")
				end)
			elseif script == pet then
				button:HookScript("OnEnter", function(b)
					self:BindUpdate(b, "PET")
				end)
			end
		end
	end
...
function AB:LoadKeyBinder()
	...
	bind:SetScript("OnEnter", function(self)
		local db = self.button:GetParent().db
		if db and db.mouseover then
			AB:Button_OnEnter(self.button)
		end
	end)
	bind:SetScript("OnLeave", function(self)
		...
		local db = self.button:GetParent().db
		if db and db.mouseover then
			AB:Button_OnLeave(self.button)
		end
	end)
	bind:SetScript("OnKeyUp", function(_, key)
		self:BindListener(key)
	end)
	...
	bind:SetScript("OnMouseWheel", function(_, delta)
		if delta > 0 then
			self:BindListener("MOUSEWHEELUP")
		else
			self:BindListener("MOUSEWHEELDOWN")
		end
	end)
