				...
				elseif frame.USE_MINI_CLASSBAR then
					if frame.CLASSBAR_DETACHED and db.classbar.orientation == "VERTICAL" then
						...
						bars[i]:Width(
							(
								CLASSBAR_WIDTH
								- ((5 + (frame.BORDER * 2 + frame.SPACING * 2)) * (frame.MAX_CLASS_BAR - 1))
							) / frame.MAX_CLASS_BAR
						) --Width accounts for 5px spacing between each button, excluding borders
					end
				elseif i ~= frame.MAX_CLASS_BAR then
					bars[i]:Width(
						(CLASSBAR_WIDTH - ((frame.MAX_CLASS_BAR - 1) * (frame.BORDER - frame.SPACING)))
							/ frame.MAX_CLASS_BAR
					) --classbar width minus total width of dividers between each button, divided by number of buttons
				end
					...
					if frame.USE_MINI_CLASSBAR then
						if frame.CLASSBAR_DETACHED and db.classbar.orientation == "VERTICAL" then
							bars[i]:Point(
								...
								"TOP",
								0,
								(db.classbar.spacing + frame.BORDER * 2 + frame.SPACING * 2)
							)
	...
	elseif frame.CLASSBAR_DETACHED then
		if frame.USE_MINI_CLASSBAR and not (frame.MAX_CLASS_BAR == 1 or frame.ClassBar == "AdditionalPower") then
			...
			if db.classbar.orientation == "HORIZONTAL" then
				bars.Holder:Size(
					db.classbar.detachedWidth + (db.classbar.spacing * widthMult) - (widthMult * 5),
					db.classbar.height
				)
			else
				bars.Holder:Size(
					db.classbar.detachedWidth,
					(db.classbar.height * UF.classMaxResourceBar[E.myclass]) + (db.classbar.spacing * widthMult)
				)
			end
		else
