	...
	if frame.ORIENTATION == "LEFT" then
		...
		if frame.USE_POWERBAR_OFFSET then
			health:Point(
				...
				frame,
				"TOPRIGHT",
				-frame.BORDER - frame.SPACING - frame.POWERBAR_OFFSET - (frame.HAPPINESS_WIDTH or 0),
				-frame.BORDER - frame.SPACING - frame.CLASSBAR_YOFFSET
			)
			health:Point(
				"BOTTOMLEFT",
				...
				frame.BORDER + frame.SPACING + frame.POWERBAR_OFFSET
			)

			health.WIDTH = health.WIDTH
				- (frame.BORDER + frame.SPACING + frame.POWERBAR_OFFSET + (frame.HAPPINESS_WIDTH or 0))
				- (frame.BORDER + frame.SPACING + frame.PORTRAIT_WIDTH)
		...
		elseif frame.USE_MINI_POWERBAR then
			health:Point(
				...
				"BOTTOMLEFT",
				frame.PORTRAIT_WIDTH + frame.BORDER + frame.SPACING,
				frame.SPACING + (frame.POWERBAR_HEIGHT / 2)
			)
...
function UF:PostUpdateHealthColor(unit, r, g, b)
	...
	if
		((colors.healthclass and colors.colorhealthbyvalue) or (colors.colorhealthbyvalue and parent.isForced))
		and not (UnitIsTapped(unit) and not UnitIsTappedByPlayer(unit))
	then
