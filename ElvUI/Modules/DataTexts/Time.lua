...
local function OnEnter(self, skipRequest)
	...
	if numSavedInstances > 0 then
		...
		for i = 1, numSavedInstances do
			...
			if name and (locked or extended) then
				...
				buttonImg = format(
					"|T%s%s:22:22:0:0:96:96:0:64:0:64|t ",
					"Interface\\LFGFrame\\LFGIcon-",
					instanceIconByName[name] or "Raid"
				)
				...
				elseif difficulty == 2 then
					tinsert(
						lockedInstances.dungeons,
						{ name, reset, extended, maxPlayers, difficultyLetter, buttonImg }
					)
				end
			end
		end
	...
	if E.db.datatexts.realmTime then
		DT.tooltip:AddDoubleLine(
			...
			GetCurrentDate(E.db.datatexts.timeFormat, true),
			1,
			1,
			1,
			lockoutColorNormal.r,
		...
		DT.tooltip:AddDoubleLine(
			...
			GetCurrentDate(E.db.datatexts.timeFormat, nil, true),
			1,
			1,
			1,
			lockoutColorNormal.r,
...
local function updateTooltipTime()
	if E.db.datatexts.realmTime then
		...
		_G["DatatextTooltipTextRight" .. DT.tooltip:NumLines()]:SetText(
			GetCurrentDate(E.db.datatexts.timeFormat, nil, true)
		)
	end
end
...
local function OnUpdate(self, elapsed)
	...
	self.text:SetText(
		gsub(
			gsub(GetCurrentDate(E.db.datatexts.timeFormat .. " " .. E.db.datatexts.dateFormat), ":", timeDisplayFormat),
			"%s",
			dateDisplayFormat
		)
	)
