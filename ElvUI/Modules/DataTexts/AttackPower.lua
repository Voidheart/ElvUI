...
local function OnEnter(self)
	...
	if E.myclass == "HUNTER" then
		...
		DT.tooltip:AddLine(
			format(RANGED_ATTACK_POWER_TOOLTIP, max(0, apower) / ATTACK_POWER_MAGIC_NUMBER),
			nil,
			nil,
			nil,
			1
		...
		DT.tooltip:AddLine(
			format(MELEE_ATTACK_POWER_TOOLTIP, max(0, apower) / ATTACK_POWER_MAGIC_NUMBER),
			nil,
			nil,
			nil,
			1
...
DT:RegisterDatatext(
	"Attack Power",
	{ "UNIT_ATTACK_POWER", "UNIT_RANGED_ATTACK_POWER" },
	OnEvent,
	nil,
	nil,
	OnEnter,
