...
local function SortDataTable(shiftKeyDown)
	if currentSortMode == shiftKeyDown and not dataUpdated then
		return
	end
...
local function BuildDataTable()
	...
	for i = 1, totalMembers do
		name, rank, rankIndex, level, _, zone, note, officernote, connected, status, englishClass =
		...
		if connected then
			...
			dataTable[#dataTable + 1] =
				{ name, rank, level, zone, note, officernote, connected, onlineStatus[status], englishClass, rankIndex }
		end
	end
...
DT:RegisterDatatext(
	"Guild",
	{ "CHAT_MSG_SYSTEM", "GUILD_ROSTER_UPDATE", "PLAYER_GUILD_UPDATE", "GUILD_MOTD" },
	OnEvent,
	nil,
	OnClick,
	OnEnter,
