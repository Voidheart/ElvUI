	...
	for charName in pairs(ElvDB.gold[E.myrealm]) do
		if ElvDB.gold[E.myrealm][charName] then
			...
			tinsert(dataTable, {
				...
				amountText = E:FormatMoney(
					ElvDB.gold[E.myrealm][charName],
					E.db.datatexts.goldFormat or "BLIZZARD",
					not E.db.datatexts.goldCoins
				),
				r = color.r,
				g = color.g,
				b = color.b,
...
local function OnEvent(self, event)
	...
	local formattedMoney =
		E:FormatMoney(curMoney, E.db.datatexts.goldFormat or "BLIZZARD", not E.db.datatexts.goldCoins)
...
local function OnEnter(self)
	...
	for _, g in ipairs(dataTable) do
		DT.tooltip:AddDoubleLine(
			g.name == E.myname and g.name .. " |TInterface\\FriendsFrame\\StatusIcon-Online:14|t" or g.name,
			g.amountText,
			g.r,
			g.g,
			g.b,
