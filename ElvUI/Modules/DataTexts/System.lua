...
local function OnEnter(self)
	...
	DT.tooltip:AddDoubleLine(
		L["Home Latency:"],
		format(homeLatencyString, homeLatency),
		0.69,
		0.31,
		0.31,
		0.84,
	...
	if cpuProfiling then
		...
		DT.tooltip:AddDoubleLine(
			L["Total CPU:"],
			format(homeLatencyString, totalCPU),
			0.69,
			0.31,
			0.31,
			0.84,
	...
	if IsShiftKeyDown() or not cpuProfiling then
		for i = 1, #memoryTable do
			...
			green = 1 - red
			DT.tooltip:AddDoubleLine(addon[2], formatMem(addon[3]), 1, 1, 1, red, green + 0.5, 0)
		end
	else
		for i = 1, #cpuTable do
			addon = cpuTable[i]
			...
			green = 1 - red
			DT.tooltip:AddDoubleLine(addon[2], format(homeLatencyString, addon[3]), 1, 1, 1, red, green + 0.5, 0)
		end
