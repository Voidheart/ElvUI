...
function A:UpdateTime(elapsed)
	...
	if self.statusBar:IsShown() and A.db.barColorGradient then
		self.statusBar:SetStatusBarColor(
			E.oUF:ColorGradient(self.timeLeft, self.duration or 0, 0.8, 0, 0, 0.8, 0.8, 0, 0, 0.8, 0)
		)
	end
...
function A:ConfigureAuras(header, auraTable, weaponPosition)
	...
	for i = 1, #auraTable do
		...
		if
			(self.db.barShow and buffInfo.duration > 0)
			or (self.db.barShow and self.db.barNoDuration and buffInfo.duration == 0)
		then
		...
		for weapon = 2, 1, -1 do
			...
			if weaponEnchantTime[weapon] then
				...
				if button then
					...
					if
						(self.db.barShow and duration > 0)
						or (self.db.barShow and self.db.barNoDuration and duration == 0)
					then
						button.statusBar:Show()

						if not button.timeLeft or not self.db.barColorGradient then
							button.statusBar:SetStatusBarColor(
								self.db.barColor.r,
								self.db.barColor.g,
								self.db.barColor.b
							)
	...
	for index = 1, display do
		...
		button.statusBar:Point(
			...
			button,
			pos,
			(isOnTop or isOnBottom) and 0
				or ((isOnLeft and -((E.PixelMode and 1 or 3) + spacing)) or ((E.PixelMode and 1 or 3) + spacing)),
			(isOnLeft or isOnRight) and 0
				or (isOnTop and ((E.PixelMode and 1 or 3) + spacing) or -((E.PixelMode and 1 or 3) + spacing))
		)
		button.statusBar:SetStatusBarTexture(LSM:Fetch("statusbar", self.db.barTexture))
...
function A:UpdateHeader(header)
	...
	repeat
		...
		aura.name, _, aura.icon, aura.count, aura.dispelType, aura.duration, aura.expires, aura.caster =
...
function A:Initialize()
	...
	self.BuffFrame:SetScript("OnUpdate", function(bf, elapsed)
		...
		if
			A:HasEnchant(1, hasMainHandEnchant, mainHandExpiration)
			or A:HasEnchant(2, hasOffHandEnchant, offHandExpiration)
		then
			A:UpdateHeader(bf)
		end
	end)
