...
do
	...
	function M:COMBAT_LOG_EVENT_UNFILTERED(_, _, event, sourceGUID, _, _, _, destName, _, _, _, _, spellID, spellName)
		if not (event == "SPELL_INTERRUPT" and (sourceGUID == E.myguid or sourceGUID == UnitGUID("pet"))) then
			return
		end
		...
		elseif E.db.general.interruptAnnounce == "EMOTE" then
			...
			if E.db.general.interruptAnnounce == "PARTY" then
				if GetNumPartyMembers() > 0 then
					SendChatMessage(
						format(interruptMsg, destName, spellID, spellName),
	...
	function M:AutoRepair(repairMode, greyValue)
		...
		if repairMode == "GUILD" then
			...
			elseif playerMoney > 0 then
				...
				if spent > 0 then
					E:Print(
						format("%s%s", L["Your items have been repaired for: "], E:FormatMoney(spent, "SMART", true))
					)
...
function M:DisbandRaidGroup()
	if InCombatLockdown() then
		return
	end -- Prevent user error in combat
...
function M:PVPMessageEnhancement(_, msg)
	if not E.db.general.enhancedPvpMessages then
		return
	end
...
function M:AutoInvite(event, leaderName)
	if not E.db.general.autoAcceptInvite then
		return
	end
