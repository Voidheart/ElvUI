...
local function buttonOnEnter(self)
	...
	for playerName, rollData in pairs(self.parent.rollResults) do
		if self.rollType == rollData[1] and rollData[2] then
			local classColor = CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[rollData[2]]
				or RAID_CLASS_COLORS[rollData[2]]
...
function M:CreateRollFrame()
	...
	if POSITION == "TOP" then
		frame:Point(
			"TOP",
			self.numFrames > 1 and self.RollBars[self.numFrames - 1] or AlertFrameHolder,
			"BOTTOM",
			0,
			-4
		)
...
function M:START_LOOT_ROLL(_, rollID, rollTime)
	...
	local texture, name, count, quality, bindOnPickUp, canNeed, canGreed, canDisenchant, reasonNeed, reasonGreed, reasonDisenchant, deSkillRequired =
