		...
		if text and match(text, "%s-%S+%s*") then
			...
			for word in gmatch(text, "%s-%S+%s*") do
				...
				if wordMatch and not E.global.chat.classColorMentionExcludedNames[wordMatch] then
					classColorTable = CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[classMatch]
	...
	if guid and guid ~= "" then
		...
		if class then
			color = (
				...
				and E:RGBToHex(CUSTOM_CLASS_COLORS[class].r, CUSTOM_CLASS_COLORS[class].g, CUSTOM_CLASS_COLORS[class].b)
			)
				or (
					RAID_CLASS_COLORS[class]
					and E:RGBToHex(RAID_CLASS_COLORS[class].r, RAID_CLASS_COLORS[class].g, RAID_CLASS_COLORS[class].b)
				)
		end
	else
		color = "|cffffffff"
...
function M:SkinBubble(frame)
	...
	name:FontTemplate(
		E.Libs.LSM:Fetch("font", E.private.general.chatBubbleFont),
		E.private.general.chatBubbleFontSize * 0.85,
		E.private.general.chatBubbleFontOutline
	)
	frame.Name = name
	...
	if E.private.general.chatBubbles == "backdrop" then
		...
		frame.text:FontTemplate(
			E.LSM:Fetch("font", E.private.general.chatBubbleFont),
			E.private.general.chatBubbleFontSize,
			E.private.general.chatBubbleFontOutline
		)
...
function M:IsChatBubble(frame)
	for i = 1, frame:GetNumRegions() do
		...
		if
			...
			and region:GetTexture() == [[Interface\Tooltips\ChatBubble-Background]]
		then
			return true
		end
	end
...
local function ChatBubble_OnEvent(self, event, msg, sender, _, _, _, _, _, _, _, _, _, guid)
	if not E.private.general.chatBubbleName then
		return
	end
