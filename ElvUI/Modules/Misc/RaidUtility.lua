...
local function CheckRaidStatus()
	...
	if
		(
			((IsRaidLeader() or IsRaidOfficer()) and GetNumRaidMembers() > 0)
			or (IsPartyLeader() and GetNumPartyMembers() > 0)
		) and not (inInstance and (instanceType == "pvp" or instanceType == "arena"))
	then
		return true
	else
		return false
...
function RU:Initialize()
	...
	self:CreateUtilButton(
		...
		(DisbandRaidButton:GetWidth() / 2) - 2,
		18,
		"TOPLEFT",
		DisbandRaidButton,
		"BOTTOMLEFT",
