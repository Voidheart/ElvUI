...
S:AddCallback("Skin_Character", function()
	if not E.private.skins.blizzard.enable or not E.private.skins.blizzard.character then
		return
	end
	...
	local popoutButtonOnEnter = function(self)
		self.icon:SetVertexColor(unpack(E.media.rgbvaluecolor))
	end
	local popoutButtonOnLeave = function(self)
	...
	hooksecurefunc("PaperDollFrameItemFlyout_DisplayButton", function(button)
		...
		if not button.location or button.location >= PDFITEMFLYOUT_FIRST_SPECIAL_LOCATION then
			return
		end
	...
	for i, button in ipairs(GearManagerDialogPopup.buttons) do
		...
		if i > 1 then
			...
			if lastPos == math.floor(lastPos) then
				button:SetPoint(
					"TOPLEFT",
					GearManagerDialogPopup.buttons[i - NUM_GEARSET_ICONS_PER_ROW],
					"BOTTOMLEFT",
					0,
					-7
				)
	...
	GearManagerDialogPopupScrollFrameScrollBar:Point(
		"BOTTOMLEFT",
	...
	for i = 1, NUM_COMPANIONS_PER_PAGE do
		...
		activeTexture:SetTexture(1, 1, 1, 0.15)
	...
	SkillDetailStatusBarUnlearnButton:HookScript("OnEnter", function(btn)
		btn.Texture:SetVertexColor(1, 1, 1)
	end)
	SkillDetailStatusBarUnlearnButton:HookScript("OnLeave", function(btn)
