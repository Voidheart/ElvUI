...
function S:HandleScrollBar(frame, horizontal)
	...
	local thumb = frame.thumbTexture
		or frame.GetThumbTexture and frame:GetThumbTexture()
		or _G[format("%s%s", frameName, "ThumbTexture")]

	if frameName then
		if not horizontal then
			scrollUpButton = parent.scrollUp
				or _G[format("%s%s", frameName, "ScrollUpButton")]
				or _G[format("%s%s", frameName, "UpButton")]
				or _G[format("%s%s", frameName, "ScrollUp")]
			scrollDownButton = parent.scrollDown
				or _G[format("%s%s", frameName, "ScrollDownButton")]
				or _G[format("%s%s", frameName, "DownButton")]
				or _G[format("%s%s", frameName, "ScrollDown")]
		else
			scrollUpButton = _G[format("%s%s", frameName, "ScrollLeftButton")]
				or _G[format("%s%s", frameName, "LeftButton")]
				or _G[format("%s%s", frameName, "ScrollLeft")]
			scrollDownButton = _G[format("%s%s", frameName, "ScrollRightButton")]
				or _G[format("%s%s", frameName, "RightButton")]
				or _G[format("%s%s", frameName, "ScrollRight")]
		end
	else
		for i, child in ipairs({ frame:GetChildren() }) do
			if child:GetObjectType() == "Button" then
				...
				elseif
					child:GetNormalTexture():GetTexture() == [[Interface\Buttons\UI-ScrollBar-ScrollDownButton-Up]]
				then
...
function S:HandleTab(tab, noBackdrop)
	if (not tab) or (tab.backdrop and not noBackdrop) then
		return
	end
...
function S:HandleEditBox(frame)
	...
	if EditBoxName then
		if _G[EditBoxName .. "Left"] then
			_G[EditBoxName .. "Left"]:SetAlpha(0)
		end
		if _G[EditBoxName .. "Middle"] then
			_G[EditBoxName .. "Middle"]:SetAlpha(0)
		end
		if _G[EditBoxName .. "Right"] then
			_G[EditBoxName .. "Right"]:SetAlpha(0)
		end
		if _G[EditBoxName .. "Mid"] then
			_G[EditBoxName .. "Mid"]:SetAlpha(0)
		end
...
function S:HandleCheckBox(frame, noBackdrop, noReplaceTextures, forceSaturation)
	...
	if not noReplaceTextures then
		...
		hooksecurefunc(frame, "SetCheckedTexture", function(checkbox, texPath)
			if texPath == E.Media.Textures.Melli or texPath == "Interface\\Buttons\\UI-CheckBox-Check" then
				return
			end
			if E.private.skins.checkBoxSkin then
...
local sliderOnDisable = function(self)
	self:GetThumbTexture():SetVertexColor(0.6, 0.6, 0.6, 0.8)
end
local sliderOnEnable = function(self)
	self:GetThumbTexture():SetVertexColor(1, 0.82, 0, 0.8)
end
	...
	if not arrowDir then
		...
		if ButtonName then
			...
			elseif find(ButtonName, "right") or find(ButtonName, "next") or find(ButtonName, "increment") then
				arrowDir = "right"
			elseif
				find(ButtonName, "scrollup")
				or find(ButtonName, "upbutton")
				or find(ButtonName, "top")
				or find(ButtonName, "promote")
			then
				arrowDir = "up"
			end
		end
...
function S:SetUIPanelWindowInfo(frame, name, value, offset, igroneUpdate, anyPanel)
	...
	if not (frameName and (anyPanel or UIPanelWindows[frameName])) then
		return
	end
	...
	if name == "UIPanelLayout-width" then
		value = E:Scale(value or (frame.backdrop and frame.backdrop:GetWidth() or frame:GetWidth()))
			+ (offset or 0)
			+ UI_PANEL_OFFSET
	end
...
function S:AddCallbackForAddon(addonName, eventName, loadFunc, forceLoad, bypass)
	...
	if
		self.addonCallbacks[addonName][eventName]
		or E.ModuleCallbacks[eventName]
		or E.InitialModuleCallbacks[eventName]
	then
		--Don't allow a registered callback to be overwritten
		E:Print(
			"Invalid argument #2 to S:AddCallbackForAddon (event name:",
			eventName,
			"is already registered, please use a unique event name)"
		)
		return
	end
...
function S:AddCallback(eventName, loadFunc)
	...
	if self.nonAddonCallbacks[eventName] or E.ModuleCallbacks[eventName] or E.InitialModuleCallbacks[eventName] then
		--Don't allow a registered callback to be overwritten
		E:Print(
			"Invalid argument #1 to S:AddCallback (event name:",
			eventName,
			"is already registered, please use a unique event name)"
		)
		return
