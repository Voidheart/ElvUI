...
function S:Ace3_SkinDropdownPullout()
	if self and self.obj then
		...
		if pullout and pullout.frame then
			if pullout.frame.template and pullout.slider.template then
				return
			end
...
function S:Ace3_RegisterAsWidget(widget)
	...
	elseif
		TYPE == "LSM30_Font"
		or TYPE == "LSM30_Sound"
		or TYPE == "LSM30_Border"
		or TYPE == "LSM30_Background"
		or TYPE == "LSM30_Statusbar"
	then
		local frame = widget.frame
...
function S:Ace3_SkinTooltip(lib, minor) -- lib: AceConfigDialog or AceGUI
	...
	if not lib or (minor and minor < minorConfigDialog) then
		return
	end
...
function S:HookAce3(lib, minor) -- lib: AceGUI
	if not lib or (not minor or minor < minorGUI) then
		return
	end
