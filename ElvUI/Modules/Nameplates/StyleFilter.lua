...
function mod:StyleFilterSetChanges(
	frame,
	actions,
	HealthColorChanged,
	BorderChanged,
...
function mod:StyleFilterClearChanges(
	frame,
	HealthColorChanged,
	BorderChanged,
	FlashingHealth,
...
function mod:StyleFilterConditionCheck(frame, filter, trigger)
	...
	if trigger.role.tank or trigger.role.healer or trigger.role.damager then
		if trigger.role[mod.TriggerConditions.roles[E:GetPlayerRole()]] then
			passed = true
		else
			return
		end
		...
		if trigger.instanceType[instanceType] then
			...
			if instanceType == "raid" or instanceType == "party" then
				...
				for _, value in pairs(D) do
					if value and not D[mod.TriggerConditions.difficulties[difficultyID]] then
						return
					end
				end
			end
	...
	if trigger.nameplateType and trigger.nameplateType.enable then
		if trigger.nameplateType[mod.TriggerConditions.frameTypes[frame.UnitType]] then
			passed = true
		else
			return
		end
	...
	then
		if trigger.raidTarget[mod.TriggerConditions.raidTargets[frame.RaidIconType]] then
			passed = true
		else
			return
		end
	...
	if frame.Buffs and trigger.buffs and trigger.buffs.names and next(trigger.buffs.names) then
		local buff = mod:StyleFilterAuraCheck(
			trigger.buffs.names,
			frame.Buffs,
	...
	if frame.Debuffs and trigger.debuffs and trigger.debuffs.names and next(trigger.debuffs.names) then
		local debuff = mod:StyleFilterAuraCheck(
...
function mod:StyleFilterPass(frame, actions)
	local healthBarEnabled = (frame.UnitType and mod.db.units[frame.UnitType].health.enable)
		or (frame.isTarget and mod.db.alwaysShowTargetHealth)
	local healthBarShown = healthBarEnabled and frame.Health:IsShown()

	mod:StyleFilterSetChanges(
		frame,
		actions,
