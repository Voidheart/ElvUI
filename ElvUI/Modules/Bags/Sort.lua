...
local function DefaultSort(a, b)
	...
	local aItemClassId, aItemSubClassId =
		itemTypes[aType] or 99, itemSubTypes[aType] and itemSubTypes[aType][aSubType] or 99
	local bItemClassId, bItemSubClassId =
		itemTypes[bType] or 99, itemSubTypes[bType] and itemSubTypes[bType][bSubType] or 99
	...
	for _, bag, slot in B:IterateBags(sourceBags, true, "withdraw") do
		...
		if itemID and targetItems[itemID] and canMove(itemID, bag, slot) then
			for i = #targetSlots, 1, -1 do
				...
				if
					bagIDs[sourceSlot]
					and bagIDs[targetedSlot] == itemID
					and targetedSlot ~= sourceSlot
					and (bagStacks[targetedSlot] ~= bagMaxStacks[targetedSlot])
					and not sourceUsed[targetedSlot]
				then
...
function B.Fill(sourceBags, targetBags, reverse, canMove)
	...
	for _, bag, slot in B:IterateBags(sourceBags, not reverse, "withdraw") do
		...
		if
			bagIDs[bagSlot]
			and B:CanItemGoInBag(bag, slot, targetBag)
			and canMove(bagIDs[bagSlot], bag, slot)
			and not blackListedSlots[bagSlot]
		then
