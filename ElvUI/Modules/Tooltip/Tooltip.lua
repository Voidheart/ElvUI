...
function TT:GameTooltip_SetDefaultAnchor(tt, parent)
	...
	if not self.db.visibility then
		return
	end
	...
	if InCombatLockdown() and self.db.visibility.combat then
		...
		if
			not (
				...
				or (modifier == "CTRL" and IsControlKeyDown())
				or (modifier == "ALT" and IsAltKeyDown())
			)
		then
			tt:Hide()
			return
	...
	if
		self.db.visibility.actionbars ~= "NONE"
		and ownerName
		and (find(ownerName, "ElvUI_Bar") or find(ownerName, "ElvUI_StanceBar") or find(ownerName, "PetAction"))
		and not keybindFrame.active
	then
	...
	if
		anchor == nil
		or (ElvUI_ContainerFrame and anchor == ElvUI_ContainerFrame)
		or anchor == RightChatPanel
		or anchor == ElvTooltipMover
		or anchor == _G.UIParent
		or anchor == E.UIParent
	then
		tt:ClearAllPoints()
...
function TT:INSPECT_TALENT_READY(event, unit)
	if not unit then
		if self.lastGUID ~= UnitGUID("mouseover") then
			return
		end
	...
	if not isShiftKeyDown and not isControlKeyDown and self.db.targetInfo then
		...
		if unit ~= "player" and UnitExists(unitTarget) then
			...
			if UnitIsPlayer(unitTarget) and not UnitHasVehicleUI(unitTarget) then
				...
				targetColor = E.db.tooltip.useCustomFactionColors
						and E.db.tooltip.factionColors[UnitReaction(unitTarget, "player")]
					or FACTION_BAR_COLORS[UnitReaction(unitTarget, "player")]
			end
		...
		if inRaid or numParty > 0 then
			for i = 1, (inRaid and numRaid or numParty) do
				...
				if not UnitIsUnit(groupUnit, "player") and UnitIsUnit(groupUnit .. "target", unit) then
					...
					tinsert(
						targetList,
						format("%s%s", E:RGBToHex(classColor.r, classColor.g, classColor.b), UnitName(groupUnit))
					)
				end
			end
			...
			if numList > 0 then
				tt:AddLine(
					format("%s (|cffffffff%d|r): %s", L["Targeted By:"], numList, tconcat(targetList, ", ")),
					nil,
					nil,
					nil,
					1
...
function TT:GameTooltipStatusBar_OnValueChanged(tt, value)
	if not value or not tt.text or not self.db.healthBar.text then
		return
	end
...
function TT:SetUnitAura(tt, ...)
	...
	if id and self.db.spellID then
		if caster then
			...
			tt:AddDoubleLine(
				format("|cFFCA3C3C%s|r %d", ID, id),
				format("%s%s", E:RGBToHex(color.r, color.g, color.b), name)
			)
		else
			tt:AddLine(format("|cFFCA3C3C%s|r %d", ID, id))
		end
...
function TT:GameTooltip_OnTooltipSetSpell(tt)
	...
	if not id or not self.db.spellID then
		return
	end
...
function TT:Initialize()
	...
	GameTooltip.StatusBar.text:FontTemplate(
		E.Libs.LSM:Fetch("font", self.db.healthBar.font),
		self.db.healthBar.fontSize,
		self.db.healthBar.fontOutline
	)
