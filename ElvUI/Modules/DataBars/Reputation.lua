...
function mod:ReputationBar_Update(event)
	if not mod.db.reputation.enable then
		return
	end
	...
	elseif name and (not self.db.reputation.hideInCombat or not self.inCombatLockdown) then
		...
		elseif textFormat == "CURMAX" then
			bar.text:SetFormattedText(
				"%s: %s - %s [%s]",
				name,
				E:ShortValue(value - minRep),
				E:ShortValue(maxRep - minRep),
				standing
			)
		...
		elseif textFormat == "CURREM" then
			bar.text:SetFormattedText(
				...
				name,
				E:ShortValue(value - minRep),
				E:ShortValue((maxRep - minRep) - (value - minRep)),
				standing
			)
	...
	if name then
		...
		GameTooltip:AddDoubleLine(
			...
			format(
				...
				value - minRep,
				maxRep - minRep,
				(value - minRep) / ((maxRep - minRep == 0) and maxRep or (maxRep - minRep)) * 100
			),
			1,
			1,
			1
...
function mod:ReputationBar_UpdateDimensions()
	...
	self.repBar.text:FontTemplate(
		LSM:Fetch("font", self.db.reputation.font),
		self.db.reputation.textSize,
		self.db.reputation.fontOutline
	)
...
function mod:ReputationBar_Load()
	self.repBar = self:CreateBar(
		"ElvUI_ReputationBar",
		self.ReputationBar_OnEnter,
		self.ReputationBar_OnClick,
		"RIGHT",
		RightChatPanel,
		"LEFT",
