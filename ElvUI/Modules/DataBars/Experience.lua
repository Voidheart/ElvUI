...
function mod:ExperienceBar_QuestXPUpdate(event)
	if event == "ZONE_CHANGED_NEW_AREA" and not self.db.experience.questXP.questCurrentZoneOnly then
		return
	end

	self.questTotalXP =
		getQuestXP(self.db.experience.questXP.questCompletedOnly, self.db.experience.questXP.questCurrentZoneOnly)
...
function mod:ExperienceBar_Update(event)
	if not mod.db.experience.enable then
		return
	end

	local bar = self.expBar
	local hideBar = (self.playerLevel == self.maxExpansionLevel and self.db.experience.hideAtMaxLevel)
		or self.expDisabled
...
function mod.ExperienceBar_OnEnter(self)
	...
	GameTooltip:AddDoubleLine(
		...
		format(
			"%d (%d%% - %d %s)",
			maxExp - curExp,
			(maxExp - curExp) / maxExp * 100,
			20 * (maxExp - curExp) / maxExp,
			L["Bars"]
		),
		1,
		1,
...
function mod:ExperienceBar_UpdateDimensions()
	...
	self.expBar.text:FontTemplate(
		LSM:Fetch("font", self.db.experience.font),
		self.db.experience.textSize,
		self.db.experience.fontOutline
	)
...
function mod:ExperienceBar_Toggle()
	if
		self.db.experience.enable
		and (self.playerLevel ~= self.maxExpansionLevel or not self.db.experience.hideAtMaxLevel)
	then
...
function mod:ExperienceBar_Load()
	self.expBar = self:CreateBar(
		"ElvUI_ExperienceBar",
		self.ExperienceBar_OnEnter,
		self.ExperienceBar_OnClick,
		"LEFT",
		LeftChatPanel,
		"RIGHT",
