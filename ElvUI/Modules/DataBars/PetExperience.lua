...
function mod:PetExperienceBar_Update(event)
	if E.myclass ~= "HUNTER" or not mod.db.petExperience.enable then
		return
	end

	local bar = self.petExpBar
	local _, hunterPet = HasPetUI()
	local hideBar = not hunterPet
		or (UnitLevel("pet") == self.maxExpansionLevel and self.db.petExperience.hideAtMaxLevel)
...
function mod:PetExperienceBar_UpdateDimensions()
	...
	self.petExpBar.text:FontTemplate(
		LSM:Fetch("font", self.db.petExperience.font),
		self.db.petExperience.textSize,
		self.db.petExperience.fontOutline
	)
...
function mod:PetExperienceBar_Load()
	...
	self.petExpBar = self:CreateBar(
		"ElvUI_PetExperienceBar",
		self.PetExperienceBar_OnEnter,
		self.PetExperienceBar_OnClick,
		"LEFT",
		LeftChatPanel,
		"RIGHT",
	...
	E:CreateMover(
		self.petExpBar,
		"PetExperienceBarMover",
		L["Pet Experience Bar"],
		nil,
		nil,
		nil,
		nil,
		nil,
		"databars,petExperience"
	)
	self:PetExperienceBar_Toggle()
end
