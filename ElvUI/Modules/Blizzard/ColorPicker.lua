...
local function GetHexColor(box)
	...
	local r, g, b =
		tonumber(strsub(rgb, 0, 2), 16) or 0, tonumber(strsub(rgb, 3, 4), 16) or 0, tonumber(strsub(rgb, 5, 6), 16) or 0
...
local function UpdateColor(box)
	if box:GetID() == 4 and box:GetNumLetters() ~= 6 then
		return
	else
		UpdateColorTexts(nil, nil, nil, box)
	end
...
function B:EnhanceColorPicker()
	...
	b:SetScript("OnClick", function()
		local color = E.myclass == "PRIEST" and E.PriestColors
