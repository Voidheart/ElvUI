...
local Easing = {}
local Callbacks =
	{ ["onplay"] = {}, ["onpause"] = {}, ["onresume"] = {}, ["onstop"] = {}, ["onreset"] = {}, ["onfinished"] = {} }
...
Update["move"] = function(self, elapsed, i)
	...
	if self.Timer >= self.Duration then
		...
		self.Parent:SetPoint(
			self.A1,
			self.P,
			self.A2,
			(self.EndX ~= 0 and self.XOffset or self.StartX),
...
Update["color"] = function(self, elapsed, i)
	...
	if self.Timer >= self.Duration then
		...
		Set[self.ColorType](
			self.Parent,
			GetColor(self.Timer / self.Duration, self.StartR, self.StartG, self.StartB, self.EndR, self.EndG, self.EndB)
		)
	end
end
