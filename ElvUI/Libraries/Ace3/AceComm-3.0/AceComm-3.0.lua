...
function AceComm:SendCommMessage(prefix, text, distribution, target, prio, callbackFn, callbackArg)
	...
	if
		not (
			...
			and type(distribution) == "string"
			and (target == nil or type(target) == "string" or type(target) == "number")
	...
	if strfind(prefix, "[\001-\009]") then
		...
		elseif not warnedPrefix then
			...
			geterrorhandler()(
				"SendCommMessage: Heads-up developers: Characters \\004--\\009 in prefix are reserved for AceComm future extension"
			)
			warnedPrefix = true
		end
	end
...
do
	...
	local function lostdatawarning(prefix, sender, where)
		DEFAULT_CHAT_FRAME:AddMessage(
			MAJOR
				.. ": Warning: lost network data regarding '"
				.. tostring(prefix)
				.. "' from '"
				.. tostring(sender)
				.. "' (in "
