	...
	* self - the ResurrectIndicator element
	...
	if UnitIsDead(unit) or UnitIsGhost(unit) then
		if
			...
			or event == "ResComm_CanRes"
			or event == "ResComm_Ressed"
			or (event == "ResComm_ResEnd" and succeeded)
		then
		...
		elseif
			(event == "ResComm_ResEnd" and not succeeded and not element.ressed) or event == "ResComm_ResExpired"
		then
			element:Hide()
			element.ressed = nil
		end
