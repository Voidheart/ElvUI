...
local isResser = (playerClass == "PRIEST")
...
local function commSend(contents, distribution, target)
	if
		not (oRA and oRA:HasModule("ParticipantPassive") and oRA:IsModuleActive("ParticipantPassive") or CT_RA_Stats)
	then
...
function lib.eventFrame:UNIT_SPELLCAST_START(unit, spellName)
	...
	if spellName ~= resSpell and spellName ~= combatResSpell then
		return
	end
...
function lib.eventFrame:UNIT_SPELLCAST_SUCCEEDED(unit, spellName)
	if unit ~= "player" or not isCasting then
		return
	end
