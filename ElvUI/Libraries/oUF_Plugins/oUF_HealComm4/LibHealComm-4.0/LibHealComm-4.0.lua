...
local byte, char, format, gsub, len, match, split =
	string.byte, string.char, string.format, string.gsub, string.len, string.match, string.split
...
local spellData, hotData, tempPlayerList, pendingHeals =
	HealComm.spellData, HealComm.hotData, HealComm.tempPlayerList, HealComm.pendingHeals
...
local isHealerClass = playerClass == "DRUID"
...
HealComm.averageHealMT = HealComm.averageHealMT
	or {
		__index = function(tbl, index)
			...
			-- This figures out how much it increases per level and how ahead of the spells level they are to figure out how much to add
				...
				average = average
					+ (UnitLevel("player") - spellLevel)
						* (spellData.increase[rank] / (MAX_PLAYER_LEVEL - spellLevel))
			end
			...
			for _, pending in pairs(spells) do
				if pending.bitType and band(pending.bitType, bitFlag) > 0 then
					for i = 1, #pending, 5 do
						...
						if
							(pending.bitType == DIRECT_HEALS or pending.bitType == BOMB_HEALS)
							and (not time or endTime <= time)
						then
						...
						elseif
							(pending.bitType == CHANNEL_HEALS or pending.bitType == HOT_HEALS)
							and (not pending.hasVariableTicks or pending.hasVariableTicks and amount[1])
						then
		...
		if pending.bitType and band(pending.bitType, bitFlag) > 0 then
			for i = 1, #pending, 5 do
				...
				if guid == filterGUID or ignoreGUID then
					...
					-- Channeled heals and hots, have to figure out how many times it'll tick within the given time band
					elseif
						(pending.bitType == CHANNEL_HEALS or pending.bitType == HOT_HEALS) and endTime > currentTime
					then
...
if playerClass == "DRUID" then
	LoadClassData = function()
		...
		itemSetsData["T10 Resto"] =
			{ 50106, 50107, 50108, 50109, 50113, 51139, 51138, 51137, 51136, 51135, 51300, 51301, 51302, 51303, 51304 }
		...
		GetHealTargets = function(bitType, guid, healAmount, spellName, hasVariableTicks)
			...
			if spellName == Tranquility then
				...
				for groupGUID, groupID in pairs(guidToGroup) do
					if
						groupID == playerGroup
						and playerGUID ~= groupGUID
						and not UnitHasVehicleUI(guidToUnit[groupID])
						and IsSpellInRange(Innervate, guidToUnit[groupGUID]) == 1
					then
			...
			elseif spellName == Lifebloom then
				...
				bombAmount = ceil(
					calculateGeneralAmount(
						hotData[spellName].levels[rank],
						hotData[spellName].bomb[rank],
						bombSpell,
						spModifier,
		...
		CalculateHealing = function(guid, spellName, spellRank)
			...
			if spellName == Regrowth then
				...
				spellPower = spellPower
					* ((spellData[spellName].coeff * 1.88) * (1 + talentData[EmpoweredRejuv].current))
			-- Nourish
			elseif spellName == Nourish then
				...
				spellPower = spellPower
					* ((spellData[spellName].coeff * 1.88) + talentData[EmpoweredTouch].spent * 0.10)
			-- Healing Touch
			...
			healAmount = calculateGeneralAmount(
				spellData[spellName].levels[rank],
				healAmount,
				spellPower,
				spModifier,
				healModifier
...
if playerClass == "PALADIN" then
	LoadClassData = function()
		...
		itemSetsData["T10 Holy"] =
			{ 50865, 50866, 50867, 50868, 50869, 51270, 51271, 51272, 51273, 51274, 51165, 51166, 51167, 51168, 51169 }
		...
		GetHealTargets = function(bitType, guid, healAmount, spellName, hasVariableTicks)
			if
				activeBeaconGUID
				and activeBeaconGUID ~= guid
				and guidToUnit[activeBeaconGUID]
				and UnitIsVisible(guidToUnit[activeBeaconGUID])
			then
...
if playerClass == "PRIEST" then
	LoadClassData = function()
		...
		spellData[BindingHeal] = {
			coeff = 1.5 / 3.5,
			levels = { 64, 72, 78 },
			averages = { avg(1042, 1338), avg(1619, 2081), avg(1952, 2508) },
			increase = { 30, 24, 7 },
		}
		-- Penance
		...
		spellData[Penance] = {
			...
			ticks = 3,
			levels = { 60, 70, 75, 80 },
			averages = { avg(670, 756), avg(805, 909), avg(1278, 1442), avg(1484, 1676) },
		}
		-- Heal
		...
		spellData[Heal] = {
			coeff = 3 / 3.5,
			levels = { 16, 22, 28, 34 },
			averages = { avg(295, 341), avg(429, 491), avg(566, 642), avg(712, 804) },
			increase = { 153, 185, 208, 207 },
		}
		-- Lesser Heal
		local LesserHeal = GetSpellInfo(2050)
		spellData[LesserHeal] = {
			levels = { 1, 4, 10 },
		...
		CalculateHealing = function(guid, spellName, spellRank)
			...
			if spellName == GreaterHeal then
				spellPower = spellPower
					* ((spellData[spellName].coeff * 1.88) * (1 + talentData[EmpoweredHealing].current))
			-- Flash Heal
			elseif spellName == FlashHeal then
				spellPower = spellPower
					* ((spellData[spellName].coeff * 1.88) * (1 + talentData[EmpoweredHealing].spent * 0.04))
			-- Binding Heal
...
if playerClass == "SHAMAN" then
	LoadClassData = function()
		...
		hotData[Riptide] =
			{ interval = 3, ticks = 5, coeff = 0.50, levels = { 60, 70, 75, 80 }, averages = { 665, 885, 1435, 1670 } }
		...
		-- Totems
		local lhwTotems =
			{ [42598] = 320, [42597] = 267, [42596] = 236, [42595] = 204, [25645] = 79, [22396] = 80, [23200] = 53 }
		...
		CalculateHealing = function(guid, spellName, spellRank)
			...
			elseif spellName == HealingWave then
				healModifier = healModifier
					* (
						talentData[HealingWay].spent == 3 and 1.25
						or talentData[HealingWay].spent == 2 and 1.16
						or talentData[HealingWay].spent == 1 and 1.08
						or 1
					)
...
if (select(2, UnitRace("player")) == "Draenei") or true then
	...
	LoadRaceData = function()
		...
		if not (naaruSpellID and naaruSpellName) then
			return
		end
...
HealComm.selfModifiers = HealComm.selfModifiers
	or {
...
HealComm.healingModifiers = HealComm.healingModifiers
	or {
...
HealComm.healingStackMods = HealComm.healingStackMods
	or {
		-- Enervating Band
		[getName(74502)] = function(name, rank, icon, stacks)
			return 1 - stacks * 0.02
		end,
		-- Tenacity
		[getName(58549)] = function(name, rank, icon, stacks)
			return icon == "Interface\\Icons\\Ability_Warrior_StrengthOfArms" and stacks ^ 1.18 or 1
		end,
		-- Focused Will
		[getName(45242)] = function(name, rank, icon, stacks)
			return 1 + (stacks * (0.02 + rankNumbers[rank] / 100))
		end,
		-- Nether Portal - Dominance
		[getName(30423)] = function(name, rank, icon, stacks)
			return 1 + stacks * 0.01
		end,
		-- Dark Touched
		[getName(45347)] = function(name, rank, icon, stacks)
			return 1 - stacks * 0.04
		end,
		-- Necrotic Strike
		[getName(60626)] = function(name, rank, icon, stacks)
			return 1 - stacks * 0.10
		end,
		-- Mortal Wound
		[getName(28467)] = function(name, rank, icon, stacks)
			return 1 - stacks * 0.10
		end,
		-- Furious Strikes
		[getName(56112)] = function(name, rank, icon, stacks)
			return 1 - stacks * 0.25
		end,
	}
...
local function loadHealList(pending, amount, stack, endTime, ticksLeft, ...)
	...
	if amount ~= -1 and amount ~= "-1" then
		...
		for i = 1, select("#", ...), 2 do
			local guid = select(i, ...)
			local amount = not pending.hasVariableTicks and tonumber((select(i + 1, ...)))
				or loadHealAmount(split("@", amount))
...
local function parseHotHeal(casterGUID, wasUpdated, spellID, tickAmount, totalTicks, tickInterval, ...)
	...
	if (IS_BUILD30300 and not totalTicks) or not tickAmount or not spellName or select("#", ...) == 0 then
		return
	end
	...
	if not stack or not duration or not endTime then
		return
	end
...
local function parseHealEnd(casterGUID, pending, checkField, spellID, interrupted, ...)
	local spellName = GetSpellInfo(spellID)
	if not spellName or not pendingHeals[casterGUID] then
		return
	end
...
function HealComm:CHAT_MSG_ADDON(prefix, message, channel, sender)
	if prefix ~= COMM_PREFIX or channel ~= distribution or sender == playerName then
		return
	end
			...
			if data.timeout then
				...
				if data.timeout <= 0 then
					...
					elseif data.type == "tick" then
						local pending = pendingHeals[casterGUID]
							and (pendingHeals[casterGUID][data.spellID] or pendingHeals[casterGUID][data.spellName])
					...
					elseif data.type == "heal" then
						local type, amount, totalTicks, tickInterval, _, hasVariableTicks =
							CalculateHotHealing(data[1], data.spellID)
						if type then
							local targets, amount = GetHealTargets(
								type,
								data[1],
								hasVariableTicks and amount or max(amount, 0),
								data.spellName,
								data,
								hasVariableTicks
							)
							...
							if not hasVariableTicks then
								sendMessage(
									format("H:%d:%d:%d::%d:%s", totalTicks, data.spellID, amount, tickInterval, targets)
								)
							else
...
function HealComm:COMBAT_LOG_EVENT_UNFILTERED(
	...
	if eventType == "SPELL_HEAL" or eventType == "SPELL_PERIODIC_HEAL" then
		...
		local pending = sourceGUID
			and pendingHeals[sourceGUID]
			and (pendingHeals[sourceGUID][spellID] or pendingHeals[sourceGUID][spellName])
	...
	elseif
		...
		) and band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == COMBATLOG_OBJECT_AFFILIATION_MINE
	then
		...
		if hotData[spellName] then
			...
			local type, amount, totalTicks, tickInterval, bombAmount, hasVariableTicks =
				CalculateHotHealing(destGUID, spellID)
			if type then
				local targets, amount = GetHealTargets(
					type,
					destGUID,
...
function HealComm:UNIT_SPELLCAST_START(unit, spellName, spellRank, id)
	if
		unit ~= "player"
		or not spellData[spellName]
		or not averageHeal[spellName][spellRank]
		or UnitIsCharmed("player")
		or not UnitPlayerControlled("player")
	then
		return
	end
...
function HealComm:UNIT_SPELLCAST_SUCCEEDED(unit, spellName, spellRank, id)
	if unit ~= "player" or not spellData[spellName] or id ~= castID or id == 0 then
		return
	end
	local nameID = spellName .. spellRank
...
function HealComm:UNIT_SPELLCAST_STOP(unit, spellName, spellRank, id)
	if unit ~= "player" or not spellData[spellName] or id ~= castID then
		return
	end
	local nameID = spellName .. spellRank
...
function HealComm:UNIT_SPELLCAST_INTERRUPTED(unit, spellName, spellRank, id)
	if unit ~= "player" or not spellData[spellName] or castID ~= id then
		return
	end
...
function HealComm:UNIT_SPELLCAST_DELAYED(unit, spellName, spellRank, id)
	...
	if
		unit == "focus"
		or unit == "target"
		or not pendingHeals[casterGUID]
		or not pendingHeals[casterGUID][spellName]
	then
		return
	end
