...
local function UpdateTooltip(self)
	GameTooltip:SetUnitAura(
		self.__unit,
		self:GetParent().aura.name,
		self:GetParent().aura.rank,
		self:GetParent().aura.filter
	)
end
	...
	for i = 1, #bars do
		...
		if bar.aura.noTime then
			...
			if element.spark == true then
				if
					element.scaleTime
					and ((element.scaleTime <= 0) or (element.scaleTime > 0 and timeleft < element.scaleTime))
				then
					bar.spark:Show()
				else
					bar.spark:Hide()
...
local function Update(self, event, unit)
	...
	if element.forceShow then
		...
		local count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate =
			5, "Magic", 0, 0, "player", nil, nil
		...
		while lastAuraIndex <= element.maxBars do
			...
			local name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellID =
				UnitAura(unit, i, helpOrHarm)
			if not name then
				break
			end

			if
				(element.filter or DefaultFilter)(
					self,
					unit,
					name,
					rank,
	...
	for i = 1, lastAuraIndex do
		...
		if bar.aura.noTime then
			...
			if element.scaleTime and element.scaleTime > 0 then
				...
				bar:Width(
					(maxValue / element.scaleTime)
						* ((element.auraBarWidth or element:GetWidth()) - (bar:GetHeight() + (element.gap or 0)))
				) -- icon size + gap
			else
