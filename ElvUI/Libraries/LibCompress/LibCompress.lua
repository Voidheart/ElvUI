...
local function getCode2(bitfield_high, bitfield, field_len)
	if field_len >= 2 then
		...
		for i = 0, field_len - 2 do
			...
			b2 = (i + 1) <= 31 and bit_band(bitfield, bit_lshift(1, i + 1))
				or bit_band(bitfield_high, bit_lshift(1, i + 1))
...
function LibCompress:GetEncodeTable(reservedChars, escapeChars, mapChars)
	...
	encode_func = assert(
		loadstring("return function(self, str) return str:gsub(self.encode_search, self.encode_translate); end")
	)()
