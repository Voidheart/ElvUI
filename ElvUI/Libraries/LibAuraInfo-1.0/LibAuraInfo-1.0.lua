...
function lib:AddAuraFromUnitID(
	dstGUID,
	name,
	texture,
	stackCount,
...
do
	...
	function CheckUnitAuras(unitID, filterType)
		...
		i = 1
		while true do
			name, _, texture, stackCount, dispelType, duration, expirationTime, unitCaster, _, _, spellID =
				UnitAura(unitID, i, filterType)
			if not name then
				break
			end
...
function lib.frame:COMBAT_LOG_EVENT_UNFILTERED(
	event,
	timestamp,
	eventType,
	srcGUID,
	...
	if lib.GUIDBlackList[dstGUID] then
		return
	end
...
function lib.frame:SPELL_AURA_REMOVED(
	event,
	timestamp,
	eventType,
	srcGUID,
...
function lib.frame:SPELL_AURA_APPLIED(
	event,
	timestamp,
	eventType,
	srcGUID,
	...
	if lib.spellDuration[spellID] then
		...
		lib:AddAuraToGUID(
			dstGUID,
			spellName,
			lib:GetSpellTexture(spellID),
			spellID,
			srcGUID,
			auraType == "DEBUFF" and "HARMFUL" or "HELPFUL"
		)
...
do
	local refreshed
	function lib.frame:SPELL_AURA_REFRESH(
		event,
		timestamp,
		eventType,
		srcGUID,
...
do
	...
	--DOSE = spell stacking
	function lib.frame:SPELL_AURA_APPLIED_DOSE(
		event,
		timestamp,
		eventType,
		srcGUID,
		...
		dosed, stackCount, expirationTime =
			lib:AddAuraDose(dstGUID, spellID, srcGUID, auraType == "DEBUFF" and "HARMFUL" or "HELPFUL")
		if dosed then
			lib.callbacks:Fire(
...
do
	local dosed, stackCount, expirationTime
	function lib.frame:SPELL_AURA_APPLIED_REMOVED_DOSE(
		event,
		timestamp,
		eventType,
		srcGUID,
		...
		dosed, stackCount, expirationTime =
			lib:RemoveAuraDose(dstGUID, spellID, srcGUID, auraType == "DEBUFF" and "HARMFUL" or "HELPFUL")
		if dosed then
			lib.callbacks:Fire(
				"LibAuraInfo_AURA_APPLIED_DOSE",
				dstGUID,
