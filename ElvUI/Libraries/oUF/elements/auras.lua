...
local function updateIcon(element, unit, index, offset, filter, isDebuff, visible)
	local name, rank, texture, count, debuffType, duration, expiration, caster, isStealable, shouldConsolidate, spellID =
	...
	if element.forceShow or element.forceCreate then
		...
		if element.forceShow then
			count, debuffType, duration, expiration, caster, isStealable, shouldConsolidate =
				5, "Magic", 0, 60, "player", nil, nil
		end
	end
	-- end Block
	...
	if name then
		...
		* show - indicates whether the aura button should be shown (boolean)
		...
		if not (element.forceShow or element.forceCreate) then
			show = (element.CustomFilter or customFilter)(
				element,
				unit,
				button,
				name,
		...
		if show then
			...
			if button.overlay then
				if
					(isDebuff and element.showDebuffType)
					or (not isDebuff and element.showBuffType)
					or element.showType
				then
...
local function UpdateAuras(self, event, unit)
	...
	if auras then
		...
		* unit - the unit for which the update has been triggered (string)
		...
		local visibleBuffs =
			filterIcons(auras, unit, auras.buffFilter or auras.filter or "HELPFUL", min(numBuffs, max), nil, 0, true)
		...
		local visibleDebuffs = filterIcons(
			auras,
			unit,
			auras.debuffFilter or auras.filter or "HARMFUL",
			min(numDebuffs, max - visibleBuffs),
			true,
			visibleBuffs
		)
		...
		* to   - the offset of the last aura button to be (re-)anchored (number)
		...
		if fromRange or auras.createdIcons > auras.anchoredIcons then
			...
			* to   - the offset of the last aura button to be (re-)anchored (number)
			--]]
			(auras.SetPosition or SetPosition)(
				auras,
				fromRange or auras.anchoredIcons + 1,
