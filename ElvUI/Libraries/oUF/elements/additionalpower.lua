...
local function UpdateColor(self, event, unit, powertype)
	if not (unit and unit == "player") and powertype == ADDITIONAL_POWER_BAR_NAME then
		return
	end
...
local function Visibility(self, event, unit)
	...
	if not UnitHasVehicleUI("player") then
		if
			(UnitPowerType("player") ~= ADDITIONAL_POWER_BAR_INDEX)
			and (UnitPowerMax("player", ADDITIONAL_POWER_BAR_INDEX) ~= 0)
		then
			shouldEnable = true
		end
	end
